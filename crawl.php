...
if (count($argv) <= 1) {
    ...
    $wid = getWebsiteID($website, $mysqli_new);
}
...
global $mysqli_new;
    $result = $mysqli_new->query("SELECT * FROM `website`");
...
function clear($wid){
	global $mysqli_new;
   $mysqli_new->query("DELETE FROM `website` WHERE aid = $wid");
}
...
function cleanWebsiteURL($url) {
	global $mysqli_new;
    $url = mysqli_real_escape_string($mysqli_new, $url);
    $prefix = "http://";
...
function scanPage($page) {
    global $website, $proxy, $mysqli_new;
